...
void Level::load_level(int offset) {
    if (offset == 0) return; // don't reload level

    if (offset == -2) { // load victory menu
    ...
    // to rewrite coin data if needed
    if (!data.empty()) encrypt();

    if (offset == -1) Player::get_instance().kill(); // kill player
